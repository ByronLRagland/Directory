...
public class AliasService {
    ...
    public List<Alias> findByName(String name){return repository.findByName(name);}
    ...
    public boolean nameIsDuplicate(Alias alias){
    ...
    public boolean personaIsDuplicate(Alias alias){
        List<Alias> aliases = findAll();
        for (Alias a : aliases){
    ...
    private Result<Alias> validate(Alias alias) {
        ...
        List<Alias> existingAliases = findByName(alias.getName());

        // Filter out the alias ID that we're validating.
        if (Validations.isNullOrBlank(alias.getPersona()) && nameIsDuplicate(alias)) {
            result.addMessage("persona is required if the alias name is already in use", ResultType.INVALID);
            if (personaIsDuplicate(alias)){
